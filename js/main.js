...
$(document).ready(function(){
	var castShowcase = $("#cast_carousel_showcase")
	var crewShowcase = $("#crew_carousel_wrap")
     castShowcase.Cloud9Carousel({
        yOrigin: 42,
        yRadius: 48,
        mirror: {
          gap: 12,
        ...
        buttonLeft: $(".cast-carousel-nav > .left"),
        buttonRight: $(".cast-carousel-nav > .right"),
        autoPlay: false,
        bringToFront: true,

        onRendered: showcaseUpdated,
        onLoaded: function(){
             castShowcase.css('visibility','visibile')
             castShowcase.css('display','none')
             castShowcase.fadeIn(1500)
        }
    })

	crewShowcase.Cloud9Carousel({
        yOrigin: 42,
        yRadius: 48,
        mirror: {
          gap: 12,
        ...
        buttonLeft: $(".crew-carousel-nav > .left"),
        buttonRight: $(".crew-carousel-nav > .right"),
        autoPlay: false,
        bringToFront: true,

        onRendered: showcaseUpdated1,
        onLoaded: function(){
             castShowcase.css('visibility','visibile')
             castShowcase.css('display','none')
             castShowcase.fadeIn(1500)
    ...
    function showcaseUpdated(showcase){
        var title = $("#cast-carousel-title").html(
            showcase.nearestItem().element.firstElementChild.firstElementChild.alt
        )

        var c = Math.cos((showcase.floatIndex() % 1) * 2 * Math.PI)
        title.css('opacity', 0.5 + (0.5 * c))
    }
    function showcaseUpdated1(showcase1){
        var title = $("#crew-carousel-title").html(
            showcase1.nearestItem().element.firstElementChild.firstElementChild.alt
        )
    ...
    $(document).keydown(function(e){
        switch(e.keyCode){
            case 37:
                $('.cast-carousel-nav > .left').click()
                break
            case 39:
                $('.cast-carousel-nav > .right').click()
        }
    })
})